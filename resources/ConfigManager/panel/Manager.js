Ext.define( 'ConfigManager.panel.Manager', {
	extend: 'Ext.Panel',
	header: false,
	layout: 'border',
	...
	requires: [
		'BS.store.BSApi',
		'ConfigManager.grid.Paths',
		'ConfigManager.panel.Configs'
	],
	minHeight: 600,
	autoHeight: true,
	paths: null,
	...
	initComponent: function() {
		...
		var me = this;
		this.pnlPath = Ext.create( 'ConfigManager.grid.Paths', {
			header: false,
			region: 'west',
			manager: me,
			flex: 2
		...
		this.pnlConfig = Ext.create( 'ConfigManager.panel.Configs', {
			region: 'center',
			store: this.store,
			manager: me,
			flex: 6
	...
	confirmDiscardChanges: function( updater ) {
		...
		bs.util.confirm(
			'configmanager-discardchanges',
			{
				...
				text: mw.message( 'configmanager-discardchanges' ).plain()
			},
			{
				ok: function() {
					me.pnlConfig.setDirty( false );
